...
- 汇编语法：
  ...
  - `LGDT`: 将内存中的 `GDT` 描述符表头加载到 `GDTR` 寄存器；
  - `LIDT`: 将内存中的 `IDT` 描述符表头加载到 `IDTR` 寄存器，**CPU 以后查段/查中断就靠这两张表**；
- `asm.bat`
  ```cmd
...
- 特殊寄存器：
  ...
  - `IDTR`: 中断记录表寄存器，保存中断记录表的起始地址等；
    ...
    | **IDTR** | 同上                              | \[15:0] **Limit**<br>\[47:16] **Base** | Limit = IDT 字节数 −1<br>Base = IDT 起始线性地址 |
